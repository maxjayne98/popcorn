...
Virtual rails are implemented with a dedicated component so scroll performance stays smooth, even when rendering hundreds of posters. Slots accept typed items so calling components can scope their own markup while the virtualization component handles layout constraints.
...
- **Node.js**: 20.16.x (LTS)  
- **pnpm**: 9.10.x  
  > If you must use npm, replace the commands below with their npm equivalents (`npm install`, `npm run dev`, etc.). The lockfile in this repo is generated by pnpm.
